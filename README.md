...
- 网络请求：Axios
- 图标资源：阿里巴巴矢量库
- 其他：Vue Router（路由管理）、localStorage（本地存储）
2.2 后端技术栈
- 框架：Spring Boot
- 认证授权：JWT
- 数据库：MySQL（数据存储）
...
- 后端发布接口：com.cy.share.controller.ContentController.publish()
- 后端更新接口：com.cy.share.controller.ContentController.update()
- 前端详情页操作弹窗：src/views/Detail/components/OperationPopup.vue
五、内容详情页模块
5.1 功能实现
1. 内容展示：
...
- 后端详情接口：com.cy.share.controller.ContentController.detail()
六、Feed 流模块
3g下看骨架屏
[图片]
6.1 功能实现
...
6.3 核心代码位置
- 前端 Feed 流页面：src/views/Feed/index.vue
- 后端 Feed 流接口：com.cy.share.controller.FeedController.list()
七、个人中心模块
个人模块新注册未发布任何log的用户骨架屏有待改善。。此处展示已发布(部署版本已修复并改变了路由访问为history模式)
[图片]
