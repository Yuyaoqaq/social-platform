...
public class LogServiceImpl extends ServiceImpl<LogMapper, Log>
        ...
        if (ok) {
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                ...
                public void afterCommit() {
                    ...
                    triggerAiTagging(log.getId(), dto.getTitle(), dto.getInfo(), dto.getPicurls());
                }
            });
        }
        return ok;
    ...
    private void triggerAiTagging(Integer logId, String title, String info, List<String> picUrls) {
        try {
            AiTagRequest req = new AiTagRequest();
            req.setLogId(logId);
            req.setTitle(title);
            req.setInfo(info);
            req.setImageUrls(picUrls);
            aiTagService.tagBlog(req); // fire-and-forget
        } catch (Exception e) {
            log.warn("AI tag request failed, logId={}", logId, e);
        }
    }
        ...
        if (ok) {
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                ...
                public void afterCommit() {
                    ...
                    triggerAiTagging(log.getId(), log.getTitle(), log.getInfo(), log.getPicurls());
                }
            });
        }
        return ok;
