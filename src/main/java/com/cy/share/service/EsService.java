...
public class EsService {
    ...
    /**
     * 搜索博客，支持 search_after 深分页。
     * title 权重 3，info 权重 1。
     *
     * @param keyword 搜索关键词
     * @param cursor  上一页游标，格式 "createTimeMs_id"，首页传 null
     * @param size    每页条数
     */
    public SearchFeedVo search(String keyword, String cursor, int size) {
        try {
            SearchSourceBuilder source = new SearchSourceBuilder()
                    .query(org.springframework.util.StringUtils.hasText(keyword)
                            ? QueryBuilders.multiMatchQuery(keyword, "title", "info")
